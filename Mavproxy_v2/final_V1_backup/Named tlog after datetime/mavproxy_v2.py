...
try:
    socket = SocketIO('http://drone.nicnepal.org', 8081, wait_for_connection=False)
...
def send_data(threadName,delay):
    ...
    while True:
        try:
            data["fix"]=mpstate.status.msgs["GPS_RAW_INT"].fix_type
            data["numSat"] = mpstate.status.msgs["GPS_RAW_INT"].satellites_visible
            data["hdop"] =  float(mpstate.status.msgs["GPS_RAW_INT"].eph)/100
            data["alt"]=float(mpstate.status.msgs['GPS_RAW_INT'].alt/1000)
            data["lat"] =float(mpstate.status.msgs["GPS_RAW_INT"].lat)/10000000
            data["lng"] = float(mpstate.status.msgs["GPS_RAW_INT"].lon)/10000000
            data["ekf_ok"]=mpstate.status.msgs["EKF_STATUS_REPORT"].compass_variance
            data["arm"]=mpstate.status.armed
            #ata["ekf"]
            data["conn"] = 'True'

            data["mode"]=mpstate.status.flightmode
            data["altr"]=mpstate.status.altitude
            data["est"]="0"
            data["head"]=mpstate.status.msgs['VFR_HUD'].heading
            data["as"]=mpstate.status.msgs['VFR_HUD'].airspeed
            data["gs"]=mpstate.status.msgs['VFR_HUD'].groundspeed
            data["volt"]=mpstate.status.msgs['SYS_STATUS'].voltage_battery
        except Exception as e:
            pass
        if  checker:#only if waypoints are successfully read
            if data["arm"] and check: # check is used to receive time of arming only once, at first arming
            ...
            if not check :
                try:
                    ...
                    vel=float((last_vel+data["gs"])/divisor) #calculate the average velocity upto current time
                    ...
                    if est<=0.5: #making sure eta is not less thatn 0.5
                        est=0
                    data["est"]=est
                    last_vel+=data["gs"] #summing up velcity to average them
        ...
        if  not checker:
            try:
                mpstate.module('wp').wp_op="list" #mission is saved as way.txt from within list
                mpstate.module('wp').master.waypoint_request_list_send()
                #for i in range(mpstate.module('wp').wploader.count()):
                time.sleep(1)
                print(mpstate.module('wp').wploader.count())
                inc=0
                for i in range(mpstate.module('wp').wploader.count()):
                    w = mpstate.module('wp').wploader.wp(i)
                    if w.command!=22:
                        waypoint[inc]= {'lat':w.x,'lng':w.y,'alt':w.z,'command':w.command}
                        inc=inc+1
                total=dis.calculate_dist(waypoint)
                checker = True
                print("waypoints",waypoint)
                #socket.emit('homePosition',waypoint[0])
            except Exception as e:
                print(e.args[0])
                print("GPS error OR no misison file received!!!")
        ...
        try:
            ...
            pass
        print "sending data at ", datetime.datetime.now()
...
def on_initiate_flight(var):
    ...
    #Guided mode
    #print("mode", mpstate.status.flightmode)

    print("fligh command received")
    while not mpstate.status.flightmode!= 'INITIALISING' and mpstate.status.msgs['GPS_RAW_INT'].fix_type > 1:
        print("initialising...")
        time.sleep(1)
        #mpstate.master.arducopter_arm()
    #mpstate.modules[10][0].cmd_mode('4')
    mpstate.master().set_mode(4)
    time.sleep(1)
    mpstate.master().arducopter_arm()

    #mpstate.modules[9][0].cmd_arm(["throttle"])
    #time.sleep(5)
    while mpstate.master().motors_armed_wait():
        print(" Waiting for arming...", mpstate.status.armed)
        time.sleep(1)
    mpstate.modules[15][0].cmd_takeoff('5')#takeoff to 5 m
    print("outside loop, flight_mission_received_about_to_fly")
    mpstate.master().set_mode(3)
    #mpstate.modules[10][0].cmd_mode('3')#auto mode
...
def set_mode_LAND(var):
    while not mpstate.status.flightmode!= 'INITIALISING' and mpstate.status.msgs['GPS_RAW_INT'].fix_type > 1:
        ...
        #mpstate.master.arducopter_arm()
    #mpstate.modules[10][0].cmd_mode('4')
    mpstate.master().set_mode(9)
    print("mode set to LAND")
    time.sleep(1)


        #print("%u %u %.10f %.10f %f p1=%.1f p2=%.1f p3=%.1f p4=%.1f cur=%u auto=%u" % ( w.command, w.frame, w.x, w.y, w.z, w.param1, w.param2, w.param3, w.param4, w.current, w.autocontinue))

def on_upload_mission(var):
    global waypoint
    waypoint={}
    mpstate.module('wp').load_waypoints(var)
    waypoint = fp.readmission(var)
...
def on_mission_download(var): #this function is called once the server requests to download the mission file, to send mission to server
    ...
    if bool(waypoints):#checking if the mission file has been downloaded from pixhawk
        print("Waypoints arer : " ,waypoints)
...
def main_loop():
    ...
    # a=0
    # for i in mpstate.modules:
    #     print(a, mpstate.modules[a])
    #     a=a+1

    #print(mpstate.mav_master)

    #master.arducopter_arm()
    #mpstate.modules[15][0].cmd_do_change_speed('5')
    ...
    while True:
        ...
        #print(mpstate.module('wp').wploader.wpoints)
        periodic_tasks()
        #print(mpstate.status.msgs['GPS_RAW_INT'].fix_type)


        #socket.wait(seconds=0.1)
        #wp = mavwp.MAVWPLoader()
        #print(dir(mpstate.status.wploader.module))
        #print(wp)
        #print(dir(mpstate.modules[3][0].load_waypoints.im_class.send_wp_requests))
        #print(mpstate.modules[3][0].fetch)
        #print(mpstate.modules[3][0].wp_op)
        #print(mpstate.modules[3][0].wploader.__dict__)
        #print(dir(mpstate.module.im_self.mav_outputs.sort))
        #print(mpstate.modules[3][0].wploader.wp(7).__dict__)
        #mission_list = mpstate.module('wp').wploader.view_list()
        #wploader = mavproxy_wp.WPModule(mpstate)
        #print(wploader.fetch())
        #wploader.save_waypoints("test.txt")
        #print(mission_list)
        #print(mpstate.module('wp').wploader.wpoints)
        #print(mpstate.module('wp').wploader.wpoints)
        #mpstate.module('wp').wp_op="list" #mission is saved as way.txt from within list
        #mpstate.module('wp').master.waypoint_request_list_send()
        #print(dir(mpstate.module('wp').master))
        #print(dir(mpstate.status.wploader.fetch))
        #print(wploader.wploader.wpoints)

        #print ("inside main ", waypoint)
        rin = []
        for master in mpstate.mav_master:
...
if __name__ == '__main__':
    ...
    parser.add_option("--logfile", dest="logfile", help="MAVLink master logfile",
                      default= str(datetime.datetime.now()) + ".tlog")
